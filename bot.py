...
def get_priority_label():
    ...
    for service in data["data"]:
        if "priority" in service.get("name", "").lower():
            return service["id"]
    priciest = max(data["data"], key=lambda x: float(x["price"].replace("$", "")))
    return priciest["id"]
...
async def button(update, context):
    ...
    if query.data == "buy":
        ...
        await query.message.reply_text(
            "This bot sells shipping labels for ⭐ Stars.\n"
            "Customer support: DM @dfdfsfewrbot on Telegram\n"
...
async def buy_label(update, context):
    ...
    await context.bot.send_invoice(
        ...
        description="Purchase a USPS Priority Mail shipping label (up to 69 lbs)",
